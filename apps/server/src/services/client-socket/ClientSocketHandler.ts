import type { Server, ServerUserSocket } from "my-planit-poker";
...
export default class ClientSocketHandler {
    ...
    setUpServerEvents(server: Server): void {
    ...
    private setUpSocketEvents(socket: ServerUserSocket): void {
        ...
        socket.on("joinRoom", (roomId, callback) => {
            try {
                const roomUser = this.handleJoinRoom.handle({
                    socket,
                    targetRoomId: roomId,
                });
                this.updateRoomStatus.handle({ socket });
                ...
                this.logger.warn(
                    "Error while joining the user in room " + roomId,
                    { error },
                );
                callback(undefined);
            }
