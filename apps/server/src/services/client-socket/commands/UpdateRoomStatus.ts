...
@injectable()
export default class UpdateRoomStatus
    implements ICommand<CommandArgs, Promise<void>>
{
    constructor(
    ...
    async handle({ socket }: CommandArgs): Promise<void> {
        ...
        const users = roomUsers.reduce<RoomUser[]>((acc, roomUser) => {
            ...
            acc.push({
                ...
                votingValue: room?.hasRevealedCards
                    ? roomUser.votingValue
                    : undefined,
