import type { SocketData } from "my-planit-poker";
...
testCommand(
    "remove user from room when it disconnects",
    ({ currentRoomUser }) => {
        ...
        expect(roomUserRepoMock.deleteByUserId).toHaveBeenCalledWith(
            testUserId,
        );
...
testCommand(
    "deletes the room if there are no more users in it",
    ({ currentRoomUser }) => {
        roomUserRepoMock.getByUserId.mockImplementation(() => currentRoomUser);
        roomUserRepoMock.getByRoomId.mockImplementation(() => []);
...
testCommand(
    "assigns a new moderator when the current one disconnects",
    ({ currentRoomUser, existingRoomUsers }) => {
